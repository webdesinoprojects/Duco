...
export default function NetbankingPanel({ paymentMethod, netbankingType }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const res = await getActiveBankDetails();
        // ✅ FIX: unwrap axios response safely
        const bank = res?.data?.data || res?.data || res || null;
        setActive(bank);
      } catch (e) {
    ...
    <div className="mt-3 rounded-xl border border-gray-200 bg-white p-4">
      {loading ? (
        ...
        <div className="space-y-2 text-sm">
          ...
          <DetailRow
            label="Account Name"
            value={active.bankdetails?.accountname || "Duco"}
          />
          <DetailRow
            label="Bank Name"
            value={active.bankdetails?.bankname}
          />
          <DetailRow
            label="Account Number"
            value={active.bankdetails?.accountnumber}
            canCopy
          />
          <DetailRow
            label="IFSC"
            value={active.bankdetails?.ifsccode}
            canCopy
          />
          <DetailRow
            label="Branch"
            value={active.bankdetails?.branch}
          />
        </div>
      )}
    </div>
